...
public class CarAdapter extends BaseAdapter implements SectionIndexer {
	...
	private List<CheXing.Car.Car1> list1 = null;
	private CheXing chexing;
	private Context mContext;
	private int i=1;

	public CarAdapter(Context mContext, List<GroupMemberBean> list) {
		this.mContext = mContext;
		this.list = list;
	}
	
	public CarAdapter(Context mContext, List<CheXing.Car.Car1> list1, CheXing chexing) {
		this.mContext = mContext;
		this.list1 = list1;
...
//	}
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		...
		CheXing.Car.Car1 car=list1.get(position);
		if (view == null) {
		...
		viewHolder.tvTitle.setText(this.list1.get(position).getFullname());
		
		if (position>0) {
			if (!list1.get(position).getParentname().equals(list1.get(position-1).getParentname())) {
				viewHolder.tvLetter.setVisibility(View.VISIBLE);
				viewHolder.tvLetter.setText(list1.get(position).getParentname());
			}else {
				viewHolder.tvLetter.setVisibility(View.GONE);
			}
		}else {
			viewHolder.tvLetter.setVisibility(View.VISIBLE);
			viewHolder.tvLetter.setText(chexing.getResult().get(0).getName());
		}
